...
DetectionState FiguresFinder::perform(Mat* frame) {
	...
	if (chessboardDetector->chessboardFound() == false)
		return DetectionState::IMPOSSIBLE;
	...
	if (isStateOK == false)
		return DetectionState::INCORRECT;
	else if (isMoveRecognized)
		return DetectionState::MOVE;
	else
		return DetectionState::UNCHANGED;
}
...
bool FiguresFinder::detectFigureMove() {
	...
	if (result == MoveRecognizer::MoveType::INCORRECT) {
		// niepoprawny stan planszy lub nierozpoznany ruch
		isStateOK = false;
		isMoveRecognized = false;
	}
	else if (result == MoveRecognizer::MoveType::NO_MOVE) {
