...
Scalar3i markerLower(70, 60, 0);
Scalar3i markerUpper(145, 255, 200);
...
ChessboardDetector::ChessboardDetector() {
	...
	if (drawWindowsLevel >= 1) {
		namedWindow(TRACKBARS_WND_NAME,0);
		createTrackbar("calibrate", TRACKBARS_WND_NAME, &calibrationOn, 1);
		createTrackbar("windows", TRACKBARS_WND_NAME, &drawWindowsLevel, 2, onDrawWindows);
		createTrackbar("result", TRACKBARS_WND_NAME, &drawResult, 1);

		createTrackbar("low H", TRACKBARS_WND_NAME, &markerLower[0], 255);
		createTrackbar("up H", TRACKBARS_WND_NAME, &markerUpper[0], 255);
		createTrackbar("low S", TRACKBARS_WND_NAME, &markerLower[1], 255);
		createTrackbar("up S", TRACKBARS_WND_NAME, &markerUpper[1], 255);
		createTrackbar("low V", TRACKBARS_WND_NAME, &markerLower[2], 255);
		createTrackbar("up V", TRACKBARS_WND_NAME, &markerUpper[2], 255);

		createTrackbar("light", TRACKBARS_WND_NAME, &globalContrast, 100);
		createTrackbar("autocontr.", TRACKBARS_WND_NAME, &autoContrast, 1);
		createTrackbar("contrast", TRACKBARS_WND_NAME, &figuresContrast, 100, onContrastChange);
		createTrackbar("white th", TRACKBARS_WND_NAME, &whiteFigureThreshold, 255, onWhiteThreshChange);
	}
}
...
void ChessboardDetector::findCorners(int method) {
	...
	inRange(hsvImg, Scalar(markerLower), Scalar(markerUpper), thresh);
